...
async def _raffle_loop(raffle_engine, openwa_client):
    ...
    while True:
        ...
        if days_until_sunday == 0 and now.hour == 0 and now.minute < 5:
            try:
                result = await raffle_engine.draw_winner(openwa_client.db)
                if result:
...
def start_raffle_scheduler(raffle_engine, openwa_client):
    global _raffle_task
    _raffle_task = asyncio.create_task(_raffle_loop(raffle_engine, openwa_client))
